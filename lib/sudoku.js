...
sudoku.createMatrix = function() {
	...
	matrix.setValue = function(columnIndex, rowIndex, value) {
		...
		if(!this.selectChoice(selectedChoice)) {
			throw new Error(util.format('Invalid value %d specified for %d column and %d row', value, columnIndex, rowIndex));
		}
	};
	return matrix;
};

sudoku.createMatrixFromString = function(givenValue) {
	var matrix = sudoku.createMatrix();

	//the length must be 81 (only 9x9 sudoku puzzles are supported)
	if(givenValue.length !== 81) {
		throw new Error('Value must be 81 characters in length. Only 9x9 puzzles are supported.\nValue: ' + givenValue);
	}

	for(var i = 0; i < givenValue.length; i++) {
		var charValue = givenValue.charAt(i),
			columnIndex = i % 9,
			rowIndex = Math.floor(i / 9);

		//only support standard 1 through 9 symbols
		//assumes any other symbol is 
		if(charValue >= '1' && charValue <= '9') {
			matrix.setValue(columnIndex, rowIndex, parseInt(charValue, 10));
		}
	}
